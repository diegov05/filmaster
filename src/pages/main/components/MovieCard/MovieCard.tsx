...
export const MovieCard: FC<Props> = (props) => {
    ...
    return (
        <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className='movie-card w-max flex flex-col justify-center items-center gap-8 transition-all'>
            ...
            >
                <img className='transition-all duration-300 hover:opacity-50 cursor-pointer rounded-3xl h-96 w-64 mt-5' src={`https://image.tmdb.org/t/p/original${movie.poster_path}`} alt={`${movie.title}`} />
                ...
                <div className="flex flex-col gap-1 w-full justify-start absolute z-10 m-4">
                    <div className='flex flex-row justify-between items-center mr-8'>
                        <div className="flex flex-row gap-2 justify-start items-center">
                            <h3 className='headtext text-xs max-w-[15ch]'>{movie.title ? movie.title : movie.name}</h3>
                    ...
                    <div className='flex flex-row gap-1'>
                        <StarRating initialValue={movie.vote_average!} size={"xtrasmall"} id={'l'} />
                        <p className='subtitle cursor-default text-xs'>({movie.vote_count})</p>
                        <p className='subtitle cursor-default max-w-[10ch] text-xs'>{movie.genres[0].name}</p>
                        <p className='subtitle cursor-default text-xs'>{movie.release_date?.slice(0, 4)}</p>
                    </div>
