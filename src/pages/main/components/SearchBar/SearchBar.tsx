        ...
        <div>
            ...
            {results.length > 0 && (
                <div className='scrollbar mt-2 bg-zinc-800 rounded-xl first-letter: h-96 w-full z-50 absolute'>
                    {results.map((movie) => (
                        <div className='p-4 flex flex-row justify-start items-start gap-4 border-b border-zinc-600' key={movie.id}>
                            <img className='w-12' src={`https://image.tmdb.org/t/p/w154/${movie.poster_path}`} alt={movie.title} />
                            <div className='flex flex-col gap-2'>
                                <p className='paragraph text-xs max-w-[12ch]'>{movie.title}</p>
                                <div className='flex flex-row justify-start items-start gap-1'>
                                    <button className='headtext py-0 px-1 text-[.5rem] custom__button bg-green-400 border-0 outline-0 text-black'>NEW</button>
                                    <button className='headtext py-0 px-1 text-[.5rem] custom__button bg-amber-400 border-0 outline-0 text-black'>POPULAR</button>
                                </div>
                            </div>
                        </div>
                    ))}
